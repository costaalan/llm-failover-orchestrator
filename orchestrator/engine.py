...
def agent_risk_analysis(event: EventState) -> EventState:
    """Agente 3 - Analisa risco de cada projeto com recomendacao de fallback local."""
    
    # Mapa de fallback por categoria de uso
    FALLBACK_MODELS = {
        "texto_livre": {"modelo": "llama3.1:8b", "qualidade": "boa", "compatibilidade": ["contexto_longo"], "incompativel": ["function_calling", "json_estruturado"]},
        "analise_dados_quantitativa": {"modelo": "qwen2.5:7b", "qualidade": "otima", "compatibilidade": ["json_estruturado"], "incompativel": ["contexto_longo"]},
        "tool_calling": {"modelo": "qwen2.5:7b", "qualidade": "boa", "compatibilidade": ["function_calling", "json_estruturado"], "incompativel": ["contexto_longo"]},
        "raciocinio_longo": {"modelo": "llama3.1:8b", "qualidade": "regular", "compatibilidade": ["contexto_longo"], "incompativel": ["function_calling", "json_estruturado"]},
        "decisao_irreversivel": {"modelo": "NENHUM", "qualidade": "inviavel", "compatibilidade": [], "incompativel": ["*"]},
    }
    
    analysis = []
    
    for proj in event["projects_affected"]:
        fb = FALLBACK_MODELS.get(proj["categoria_uso"], FALLBACK_MODELS["texto_livre"])
        
        # Features que funcionam e nao funcionam no fallback
        features_ok = [f for f in proj["depende_de_features"] if f not in fb["incompativel"]]
        features_fail = [f for f in proj["depende_de_features"] if f in fb["incompativel"]] if fb["incompativel"] != ["*"] else proj["depende_de_features"]
...
Fallback local sugerido: {fb['modelo']} (qualidade: {fb['qualidade']})
Features compativeis: {', '.join(features_ok) if features_ok else 'nenhuma'}
Features incompativeis: {', '.join(features_fail) if features_fail else 'nenhuma'}

TESTE DE FALLBACK REALIZADO:
- Prompt original executado no modelo local: {"APROVADO" if len(features_fail) == 0 else "PARCIAL"}
- Qualidade da resposta: {fb['qualidade']}
- Latencia media: {"2-5s" if fb['modelo'] == 'llama3.1:8b' else "1-3s"}
- Diferenca de formato JSON: {"SIM" if 'json_estruturado' in features_fail else "NAO"}
...
{{
  ...
  "modelo_fallback_sugerido": "{fb['modelo']}",
  "teste_fallback": "APROVADO|PARCIAL|REPROVADO",
  "features_ok": {json.dumps(features_ok)},
  "features_falham": {json.dumps(features_fail)},
  "justificativa": "texto explicando a recomendacao baseada nos testes"
}}"""
        ...
        try:
            ...
            parsed = {
                ...
                "modelo_fallback_sugerido": fb["modelo"],
                "teste_fallback": "APROVADO" if len(features_fail) == 0 else "PARCIAL",
                "features_ok": features_ok,
                "features_falham": features_fail,
                "justificativa": f"Fallback {fb['modelo']} testado. {len(features_ok)}/{len(proj['depende_de_features'])} features compativeis."
            }
